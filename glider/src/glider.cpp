...
unsigned char **glider_1_png = (unsigned char **) 0x137c58; // std::string
...
static void mcpi_callback(unsigned char *minecraft){
    ...
    if (player != NULL){
        ...
        if (item == NULL) return;
        if (item->id == 404){
            *(std::string*) glider_1_png = "armor/glider_1.png";
        } else {
            *(std::string*) glider_1_png = "armor/iron_1.png";
        }
    }
}
...
// Custom glider item
unsigned char *glider;
unsigned char *make_glider(){
    // glider
    ...
    (*Item_setDescriptionId)(item, "glider");
...
// Add glider to creative inventory
static void Inventory_setupDefault_FillingContainer_addItem_call_injection(unsigned char *filling_container) {
    ItemInstance *glider_instance = new ItemInstance;
    ALLOC_CHECK(glider_instance);
    glider_instance->count = 255;
    glider_instance->auxiliary = 0;
    glider_instance->id = 404;
    (*FillingContainer_addItem)(filling_container, glider_instance);
}
